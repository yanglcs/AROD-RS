...
import warnings
from utils.utils import _jensen_shannon_div
from utils.utils import _kl_div
...
def train():
    ...
    if args.dataset == 'COCO':
        ...
        image_sets=process_names_voc(args.data_use, type='trainval')[0]
        logger.info(f'training datasets {image_sets}')
        dataset = hrscDetection(root=args.dataset_root, image_sets=image_sets,
                               transform=SSDAugmentation(cfg['min_dim']))
    ...
    if args.resume:
        ...
        ssd_net.load_weights_expand(args.resume)
    ...
    for iteration in range(args.start_iter//pgd.iters, args.max_iter//pgd.iters):
        ...
        for im_clean, at_img, at_targets in adv_generator:
            ...
            with amp.autocast(enabled=args.amp):
                if args.adc > 0:
                    adv_pred = net.disc(torch.cat((images, at_img)))#.module.
                ...
                if args.cfr:
                    ...
                    cr_loss = _jensen_shannon_div(at_img, im_clean)
                    ...
                    #loss = loss + (recons_loss * 1e-5 + kld_loss * 1e-15) / 255.0 #recons_loss * 0.16+ kld_loss * 5
                    #loss = loss + (recons_loss * 1e-2 + kld_loss * 1e-12) / 255.0  #
                    #loss = loss + (recons_loss *0.16  + kld_loss * 1e-12 + cr_loss*5) / 255.0  #720000
                    loss = loss + (recons_loss *0.16 + kld_loss * 5 + cr_loss*5) / 255.0  #720000 +kld_loss * 5 
            ...
            if inter_iteration % 40 == 0: #50
                t1 = time.time()
                if args.cfr:
                    logger.info('iter ' + repr(inter_iteration) + ' || Loss: %.4f || recons_loss: %.4f || kld_loss: %.4f || cr_loss: %.4f ||timer: %.4f sec.' %
                                (sum_loss / 40, recons_loss.item(), kld_loss.item(), cr_loss.item(), (t1 - t0) / 40))
                else:
                    logger.info('iter ' + repr(inter_iteration) + ' || Loss: %.4f || timer: %.4f sec.' % (
                    sum_loss / 40, (t1 - t0) / 40))
                t0 = time.time()
                sum_loss = 0
...
def weights_init(m):
    if isinstance(m, nn.Conv2d) or isinstance(m, ODConv2d):
