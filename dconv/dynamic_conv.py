...
class ODConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bias=True, stride=1, padding=0, dilation=1, groups=1,
        ...
        self.weight = nn.Parameter(torch.randn(kernel_num, out_channels, in_channels//groups, kernel_size, kernel_size),
                                   requires_grad=True)
        self.bias = nn.Parameter(torch.Tensor(kernel_num, out_channels))
        self._initialize_weights()
    ...
    def _forward_impl_common(self, x, softmax_attention=None):
        ...
        #aggregate_bias =  spatial_attention * kernel_attention * self.bias.unsqueeze(dim=0)
        #aggregate_bias = torch.sum(aggregate_weight, dim=1).view(1,-1)
    ...
    def _forward_impl_pw1x(self, x, softmax_attention=None):
        channel_attention, filter_attention, spatial_attention, kernel_attention = self.attention(x)
        x = x * channel_attention
    ...
    def forward(self, x, softmax_attention=None):
        return self._forward_impl(x, softmax_attention=None)
