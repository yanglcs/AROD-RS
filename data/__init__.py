...
from .hrsc import hrscDetection, hrscAnnotationTransform, hrsc_CLASSES
...
def process_names_hrsc(namestr, type=None, year='2007'):
    types_set = namestr.split('/')
    ...
    def proc(k, x):
        st = x.find('[')
        if st != -1:
            return (x[st + 1:x.find(']', st + 1)], k, dataset_names['hrsc'][x[:st]])
        else:
            return (year, k, dataset_names['hrsc'][x])
...
def detection_collate(batch):
    ...
    #print(batch[0].keys())
    #for x in batch:
        #x['target'] = x['target'].astype(np.float64)
        #print(x)
        ...
        #    print (type(y))
    #    for y in x:
       #np.array(x.values(),dtype=float64) 
