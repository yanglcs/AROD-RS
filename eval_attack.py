...
def get_hrsc_results_file_template(image_set, cls):
    # VOCdevkit/VOC2007/results/det_test_aeroplane.txt
    filename = f'det_{exp_name}_' + image_set + '_%s.txt' % (cls)
    filedir = os.path.join(devkit_path, 'results')
    if not os.path.exists(filedir):
...
def write_voc_results_file(all_boxes, dataset):
    for cls_ind, cls in enumerate(voc_labelmap):
...
def write_hrsc_results_file(all_boxes, dataset):
    for cls_ind, cls in enumerate(['ship']):#'hrsc_labelmap"
        #print(cls)
        logger.info('Writing {:s} hrsc results file'.format(cls))
        filename = get_hrsc_results_file_template(set_type, cls)
        with open(filename, 'wt') as f:
            for im_ind, index in enumerate(dataset.ids):
                dets = all_boxes[cls_ind+1][im_ind]
                #print(dets)
...
def do_python_eval(output_dir='output', use_07=True):
    ...
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)
    score_list=[]
    for i, cls in enumerate(['ship']): #hrsc
        filename = get_hrsc_results_file_template(set_type, cls)#hrsc
    ...
    with open(f'./confidence/confidence_{exp_name}.pkl', 'wb') as f:
...
def test_net_attack(save_folder, net, cuda, dataset, adv_type):
    ...
    #cv2.imwrite(save_folder +  + '_adv.jpg', adv_generator)
    ...
    with torch.no_grad():
        for i in range(num_images):
            ...
            #cv2.imwrite('{}.png'.format(os.path.join(save_folder,'recons', str(i))), (net.recons.detach().cpu().squeeze(0)+torch.tensor(DATASET_MEANS, device='cpu').view(-1,1,1)).permute(1,2,0).numpy()[:,:,(2,1,0)])
    ...
    if args.dataset=='VOC' :
        ...
        evaluate_detections_hrsc(all_boxes, output_dir, dataset)
   
def evaluate_detections_voc(box_list, output_dir, dataset):
    write_voc_results_file(box_list, dataset)
    do_python_eval(output_dir)

def evaluate_detections_hrsc(box_list, output_dir, dataset):
    write_hrsc_results_file(box_list, dataset)
    do_python_eval(output_dir)
...
if __name__ == '__main__':
    ...
    '''''''''
    trained_model.pop('conf.0.weight')
    trained_model.pop('conf.1.weight')
    trained_model.pop('conf.2.weight')
    trained_model.pop('conf.3.weight')
    trained_model.pop('conf.4.weight')
    trained_model.pop('conf.5.weight')
    trained_model.pop('conf.0.bias')
    trained_model.pop('conf.1.bias')
    trained_model.pop('conf.2.bias')
    trained_model.pop('conf.3.bias')
    trained_model.pop('conf.4.bias')
    trained_model.pop('conf.5.bias')
    trained_model.pop('conf.0.attention.filter_fc.weight')
    trained_model.pop('conf.0.attention.filter_fc.bias')
    trained_model.pop('conf.1.attention.filter_fc.weight')
    trained_model.pop('conf.1.attention.filter_fc.bias')
    
    trained_model.pop('loc.3.weight')
    trained_model.pop('loc.2.weight')
    trained_model.pop('loc.2.bias')
    trained_model.pop('loc.3.bias')
    trained_model.pop('loc.4.weight')
    trained_model.pop('loc.4.bias')
    trained_model.pop('loc.5.weight')
    trained_model.pop('loc.5.bias')
    '''''''''
    
    
    net.load_state_dict(trained_model, strict=False)
    #net.load_weights_expand(args.trained_model)
    

    
    #net.load_state_dict(torch.load(trained_model), strict=False)
    net.eval()
    ...
    # evaluation 
    if args.dataset == 'VOC':
        eval_folder=(process_names_voc)(args.data_use, type='test')
    elif args.dataset == 'hrsc':
        eval_folder=(process_names_hrsc)(args.data_use, type='test')
        #print(eval_folder)
    elif args.dataset == 'coco':
        eval_folder=(process_names_coco)(args.data_use, type='test')
    ...
    for dadv in eval_folder:
        ...
        if args.dataset == 'VOC':
            ...
            #print(dataset_adv)
             ...
             dataset_adv = hrscDetection(args.dataset_root, dadv, BaseTransform(300),
                                       hrscAnnotationTransform(),
                                       give_size=True)
