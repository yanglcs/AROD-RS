...
def add_extras(cfg, i, conv_layer, batch_norm=False):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S':
                layers += [conv_layer(in_channels, cfg[k + 1], kernel_size=(1, 3)[flag], stride=2, padding=1)] #conv_layer 将extra阶段的conv替换成ori-conv
            else:
                layers += [conv_layer(in_channels, v, kernel_size=(1, 3)[flag])] #conv_layer
            flag = not flag
        in_channels = v
    return layers
...
def multibox(conv_layer, vgg, extra_layers, cfg, num_classes):
    loc_layers = []
    conf_layers = []
    vgg_source = [17, -2] #21 17
    for k, v in enumerate(vgg_source):
       
        loc_layers += [conv_layer(vgg[v].out_channels, cfg[k] * 4, kernel_size=3, padding=1)]#,* 4
        conf_layers += [conv_layer(vgg[v].out_channels, cfg[k] * num_classes, kernel_size=3, padding=1)]# * 4
    for k, v in enumerate(extra_layers[1::2], 2):
        loc_layers += [nn.Conv2d(v.out_channels, cfg[k] * 4, kernel_size=3, padding=1)]#, padding=1 conv_layer # * 4
        conf_layers += [nn.Conv2d(v.out_channels, cfg[k] * num_classes, kernel_size=3, padding=1)]#, padding=1 conv_layer
...
def add_extras_convresnet(conv_layer, input_size=[1024, 512, 512, 256, 256, 256]):
    # Extra layers added to VGG for feature scaling
    additional_blocks = []  # 存放额外卷积层的列表
    # input_size = [1024, 512, 512, 256, 256, 256] for resnet50
    middle_channels = [256, 256, 128, 128, 128]
...
def multibox_odresnet(conv_layer, resnet, extra_layers, cfg, num_classes):
    loc_layers = []
    conf_layers = []
