from dconv.dynamic_conv import ODConv2d
from models.base import ProbConv2d
import torch
...
class RobustSSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, disc, num_classes, dconv_K=4, decoder=None, backbone='vgg', CFR_layer=21):
        ...
        for layer in self.modules():
            if isinstance(layer, ODConv2d) or (isinstance(layer, ProbConv2d) and isinstance(layer.conv_mean, ODConv2d)):
    ...
    def load_weights_expand(self, base_file, weights_init=None):
        ...
        other, ext = os.path.splitext(base_file)
        if ext in ['.pkl', '.pth']:
            print('Loading Conv weights expand into ODConv2d layer...')
            ...
            for k, v in load_dict.items():
                ...
                if k in self_dict.keys() or k.startswith('vgg'):
                    ...
                    if isinstance(layer, ODConv2d) or (isinstance(layer, ProbConv2d) and isinstance(layer.conv_mean, ODConv2d)):  
                        v = v.unsqueeze(0).repeat([layer.kernel_num] + torch.ones(len(v.shape), dtype=int).tolist())
                    if isinstance(layer, ProbConv2d):
                        ...
                        state_dict[k]=v                                                                                                      
                """""""""#end resume CFR=true weight
                if k in self_dict.keys() or k.startswith('vgg'):
                    if (k.startswith('vgg') or k.startswith('extras')) and v.dim() != 0: #or k.startswith('loc') 
                        v = torch.narrow(v, 0, 0, 1)
                        #v = torch.narrow(v, 1, 0, 1)
                        v = torch.squeeze(v,0)
                        layer=module_dict[k[:k.rfind('.')]]
            ...
            state_dict.pop('conf.0.weight')
            state_dict.pop('conf.1.weight')
            state_dict.pop('conf.0.bias')
            state_dict.pop('conf.1.bias')
            state_dict.pop('loc.0.weight')
            state_dict.pop('loc.1.weight')
            state_dict.pop('loc.0.bias')
            state_dict.pop('loc.1.bias')

            """""""""   
            state_dict.pop('conf.0.weight')
            state_dict.pop('conf.1.weight')
            state_dict.pop('conf.2.weight')
            state_dict.pop('conf.3.weight')
            state_dict.pop('conf.4.weight')
            state_dict.pop('conf.5.weight')
            state_dict.pop('conf.0.bias')
            state_dict.pop('conf.1.bias')
            state_dict.pop('conf.2.bias')
            state_dict.pop('conf.3.bias')
            state_dict.pop('conf.4.bias')
            state_dict.pop('conf.5.bias')
