from dconv.dynamic_conv import DynamicConv2d, ODConv2d
from dconv.od_resnet import od_resnet18
from .base import *
from .ssd import *
...
def build_robust_ssd(phase, size=300, num_classes=21, amp=False, CFR=False, CFR_layer=21, multi_fc=False, K=4, backbone='vgg'):
    ...
    conv_layer1 = lambda *args, **kwargs: ODConv2d(*args, **kwargs)
    conv_layer2 = lambda *args, **kwargs: nn.Conv2d(*args, **kwargs)
    if backbone == 'vgg':
        base_, extras_, head_ = multibox(conv_layer1, vgg(base[str(size)], 3, conv_layer2),
                                         add_extras(extras[str(size)], 1024, conv_layer2),                #add_extras(extras[str(size)], 1024, conv_layer),  
                                         mbox[str(size)], num_classes)
    elif backbone=='resnet':
        base_, extras_, head_ = multibox_resnet(conv_layer1, resnet(conv_layer2),      #resnet(conv_layer),
                                         add_extras_resnet(conv_layer2),
                                         mbox[str(size)], num_classes)
    elif backbone=='od_resnet18':
        base_, extras_, head_ = multibox_odresnet(conv_layer1, od_resnet18(conv_layer1),      #resnet(conv_layer),
                                         add_extras_convresnet(conv_layer1),
                                         mbox[str(size)], num_classes)
